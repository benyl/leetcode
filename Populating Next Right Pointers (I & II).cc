 ...
 */
 ...
 // ==============================================================
// version 1, iterative version, use queue for BFS
class Solution {
public:
...
// ==============================================================
// version 2, recursive version, use recursive stack for DFS
class Solution {
public:
    void connect(TreeLinkNode *root) {
        if(!root) return;
        
        TreeLinkNode *last= (root->right) ? root->right : root->left;
        if(!last) return;
        ...
        while(next!=NULL && !next->left && !next->right)
            next = next->next;
        if(next)
            last->next = (next->left) ? next->left : next->right;
         
        connect(root->right); // connect right side first
        connect(root->left);
    }
};